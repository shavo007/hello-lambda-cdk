...
AWS credentials must be configured (`aws configure` or env vars) before any `cdk` command.
...
- **`lambda/`** — Container image context: multi-stage `Dockerfile` using `python:3.13-slim` as the custom base image with `awslambdaric` (Lambda runtime interface client), plus `handler.py`
- **`cdk.json`** — Tells CDK CLI to run via `uv run python3 app.py`
- **`DOCKER_IMAGE.md`** — Changelog tracking Docker image evolution (v1: AWS base image → v2: custom base image)
...
- Custom base images require `awslambdaric` as the `ENTRYPOINT` — without it Lambda cannot invoke the handler
